export const statsData = [
  {
    title: "Total Pelatihan",
    description: "Jumlah pelatihan aktif",
    lastUpdate: "campaign sent 2 days ago",
    chartData: [2, 4, 3, 6, 1, 2, 5],
    chartLabels: ["M", "T", "W", "T", "F", "S", "S"],
  },
  {
    title: "Total Peserta",
    description: "Peserta terdaftar",
    lastUpdate: "updated 4 min ago",
    chartData: [3, 2, 4, 1, 6, 3, 2, 4, 1, 3, 5, 2],
    chartLabels: ["Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
  },
  {
    title: "LPK Terdaftar",
    description: "Lembaga pelatihan kerja",
    lastUpdate: "just updated",
    chartData: [2, 4, 3, 5, 1, 3, 6, 2, 4, 1, 3, 2],
    chartLabels: ["Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"],
  },
];

export const ordersOverview = [
  { icon: "bell", iconColor: "text-green-500", title: "$2400, Design changes", time: "22 DEC 7:20 PM" },
  { icon: "shopping-cart", iconColor: "text-red-500", title: "New order #1832412", time: "21 DEC 11 PM" },
  { icon: "credit-card", iconColor: "text-blue-500", title: "Server payments for April", time: "21 DEC 9:34 PM" },
  { icon: "plus", iconColor: "text-orange-500", title: "New card added for order #4395133", time: "20 DEC 2:20 AM" },
  { icon: "package", iconColor: "text-purple-500", title: "Unlock packages for development", time: "18 DEC 4:54 AM" },
];

export const projectsData = [
  {
    id: "1",
    name: "Material XD Version",
    icon: "palette",
    iconBg: "bg-gray-900",
    members: [
      { id: "1", name: "Alex Johnson", avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&h=150&fit=crop&crop=face", color: "bg-blue-500" },
      { id: "2", name: "Sarah Chen", avatar: "https://images.unsplash.com/photo-1494790108755-2616b612b786?w=150&h=150&fit=crop&crop=face", color: "bg-green-500" },
      { id: "3", name: "Michael Brown", avatar: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=150&h=150&fit=crop&crop=face", color: "bg-yellow-500" },
    ],
    budget: "$14,000",
    completion: 60,
  },
  {
    id: "2",
    name: "Add Progress Track",
    icon: "chart-line",
    iconBg: "bg-blue-500",
    members: [
      { id: "5", name: "David Wilson", avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&h=150&fit=crop&crop=face", color: "bg-purple-500" },
      { id: "6", name: "Jessica Martinez", avatar: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=150&h=150&fit=crop&crop=face", color: "bg-pink-500" },
    ],
    budget: "$3,000",
    completion: 10,
  },
  {
    id: "3",
    name: "Fix Platform Errors",
    icon: "bug",
    iconBg: "bg-red-500",
    members: [
      { id: "7", name: "Ryan Thompson", avatar: "https://images.unsplash.com/photo-1519244703995-f4e0f30006d5?w=150&h=150&fit=crop&crop=face", color: "bg-indigo-500" },
      { id: "8", name: "Lisa Garcia", avatar: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&h=150&fit=crop&crop=face", color: "bg-teal-500" },
    ],
    budget: "Not set",
    completion: 100,
  },
  {
    id: "4",
    name: "Launch our Mobile App",
    icon: "smartphone",
    iconBg: "bg-green-500",
    members: [
      { id: "9", name: "James Rodriguez", avatar: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?w=150&h=150&fit=crop&crop=face", color: "bg-orange-500" },
      { id: "10", name: "Maria Lopez", avatar: "https://images.unsplash.com/photo-1487412720507-e7ab37603c6f?w=150&h=150&fit=crop&crop=face", color: "bg-cyan-500" },
    ],
    budget: "$20,500",
    completion: 100,
  },
  {
    id: "5",
    name: "Add the New Pricing Page",
    icon: "tag",
    iconBg: "bg-blue-600",
    members: [
      { id: "13", name: "User 13", avatar: "", color: "bg-gray-800" },
    ],
    budget: "$500",
    completion: 25,
  },
  {
    id: "6",
    name: "Redesign New Online Shop",
    icon: "shopping-bag",
    iconBg: "bg-pink-500",
    members: [
      { id: "14", name: "User 14", avatar: "", color: "bg-violet-500" },
      { id: "15", name: "User 15", avatar: "", color: "bg-amber-500" },
    ],
    budget: "$2,000",
    completion: 40,
  },
];

export const areaChartData = [
  { month: "Jan", sales: 24000, expenses: 12400 },
  { month: "Feb", sales: 18500, expenses: 8750 },
  { month: "Mar", sales: 32000, expenses: 15800 },
  { month: "Apr", sales: 27800, expenses: 13200 },
  { month: "May", sales: 21900, expenses: 9600 },
  { month: "Jun", sales: 29500, expenses: 16300 },
];

export const lineChartData = [
  { day: "Mon", users: 24, sessions: 45 },
  { day: "Tue", users: 13, sessions: 32 },
  { day: "Wed", users: 98, sessions: 67 },
  { day: "Thu", users: 39, sessions: 54 },
  { day: "Fri", users: 48, sessions: 78 },
  { day: "Sat", users: 38, sessions: 56 },
  { day: "Sun", users: 43, sessions: 65 },
];

export const pieChartData = [
  { name: "Bekerja", value: 65, fill: "#1E88E5" },
  { name: "Belum Bekerja", value: 28, fill: "#2E7D32" },
  { name: "Wirausaha", value: 7, fill: "#90CAF9" },
];

export const barChartData = [
  { category: "Q1", revenue: 120000, profit: 48000 },
  { category: "Q2", revenue: 190000, profit: 76000 },
  { category: "Q3", revenue: 155000, profit: 62000 },
  { category: "Q4", revenue: 225000, profit: 90000 },
];